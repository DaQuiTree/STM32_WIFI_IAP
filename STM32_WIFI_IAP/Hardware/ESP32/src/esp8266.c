...
void Simple_SendString(char *str, u32 time)
{
    ESP8266_Fram_Record_Struct.FramLength = 0;
    ESP8266_USART("%s\r\n", str);
    while(!ESP8266_Fram_Record_Struct.FramFinishFlag);
    delay_ms(time);
    
    ESP8266_Fram_Record_Struct.Data_RX_BUF[ESP8266_Fram_Record_Struct.FramLength] = '\0';
    printf("echo: %s",ESP8266_Fram_Record_Struct.Data_RX_BUF);
    
    ESP8266_Fram_Record_Struct.FramFinishFlag = 0;
}
...
{ 
    ...
    if(ack1!=0&&ack2!=0)
    {
        res = (bool)strstr(ESP8266_Fram_Record_Struct.Data_RX_BUF, ack1) || (bool)strstr(ESP8266_Fram_Record_Struct.Data_RX_BUF, ack2);
        return res;
    }else if( ack1 != 0 ){  //strstr(s1,s2);检测s2是否为s1的一部分，是返回该位置，否则返回false，它强制转换为bool类型了
        res = (bool)strstr(ESP8266_Fram_Record_Struct.Data_RX_BUF, ack1);
        return res;
    }else{
        res = (bool)strstr(ESP8266_Fram_Record_Struct.Data_RX_BUF, ack2);
        return res;
    }
}
...
void ESP8266_AT_Test(void)
{
    ...
    while(count < 10)
    {
        if(ESP8266_Send_AT_Cmd("AT+RESTORE","OK",NULL,2000)) 
        {
            printf("OK\r\n");
            return;
        }
...
bool ESP8266_NormalSend ( void )
{
    return ESP8266_Send_AT_Cmd( "AT+CIPSEND=10", "OK", ">", 500 );
}
...
bool ESP8266_SendString(FunctionalState enumEnUnvarnishTx, char *pStr, u32 ulStrLength, ENUM_ID_NO_TypeDef ucId )
{
    ...
    if(enumEnUnvarnishTx){
        ...
        ESP8266_Send_AT_Cmd ( cStr, "OK", ">", 1000 );
        Simple_SendString(pStr, 1000);
        
        //ESP8266_Send_AT_Cmd(pStr, "SEND OK", 0, 1000 );
    }
...
bool ESP8266_RecieveData(u8 **buf, int *plen, const int wantedsz)
{
    int secLen = 0;
    char *p;
    int len;
    
    while(!ESP8266_Fram_Record_Struct.FramFinishFlag);// 等待flag
    //delete +IPD,xxxx:
    p = strchr((char *)ESP8266_Fram_Record_Struct.Data_RX_BUF, ':');
    *p = '\0';
    printf("%s\r\n", ESP8266_Fram_Record_Struct.Data_RX_BUF);
    secLen = strlen(ESP8266_Fram_Record_Struct.Data_RX_BUF);
    *buf = ++p; 
    
    len = ESP8266_Fram_Record_Struct.FramLength - secLen - 3;
    len == wantedsz ? (*plen = wantedsz) : (*plen = len);
